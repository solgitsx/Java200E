...
public class BillboardfxController {
 ...
 void initialize() {
  ...
  lastweek.setCellValueFactory(cellData-> 
                              cellData.getValue().lastweekProperty().asObject());
    	...
    	if(billobardTable.getSelectionModel().getSelectedItem()!=null ){
    		...
    		webEngine.load(
    		   "https://www.youtube.com/results?search_query="
    		      +toP(billboard.getArtist())+toP(billboard.getSong()));
    	}
      }
    );
	getAllBillboard();
 ...
 public void getAllBillboard(){
	...
	rs=RestDay.toWantedDay(rs, count); //count 1 : 1주흐
	...
	lbbill.setText("Billboard Chat 200 "+rs);  
 }
