...
import kr.co.infopub.chapter.s197.dto.Department;
import kr.co.infopub.chapter.s197.dto.EmpConvert;
...
public class EmployeeSearchFxController extends BorderPane {
	    @FXML
		...
		public EmployeeSearchFxController(EmployeeService service) {
			this.service=service;
			
			FXMLLoader loader4 = new FXMLLoader();
	        loader4.setLocation(Main.class.getResource("view/EmployeeSearchFx.fxml"));
	        loader4.setRoot(this);
	        loader4.setController(this);
	        try {
	        	loader4.load();
	        } catch (Exception exception) {
	        	System.out.println("EmployeeSearchFxController 로드 실패");
	    ...
	    void actionAllEmployee(ActionEvent event) {
	    	 try {
	 	    	...
	 	    	ObservableList<Employee> updatesearchnameList=service.findManagersByName(searchname);
	  ...
	  //depart(59) ==> depart
	    ...
	    void showJobTable(ObservableList<String>jobs){
	    	if(cbJobid!=null){
...
//	    }
	    ...
	    void actionAddEmployee(ActionEvent event) {
	    	...
	    	try {
				...
				int empid=service.addEmployee(empdto);
				if(empid>0){
	    ...
	    void actionSearchManger(ActionEvent event) {
			...
			try {
				managers=service.findManagersByName(searchManagerId);
