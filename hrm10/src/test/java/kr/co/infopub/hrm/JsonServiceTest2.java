...
public class JsonServiceTest2 {
 ...
 public void test() {
	...
	this.restTemplate.getForEntity("/api/findAllDepartments", List.class);
	List<LinkedHashMap<String, Object>> usersMap =res.getBody();
	System.out.println(usersMap.get(0));
	Assert.assertEquals(usersMap.get(0).get("department_id").toString(),"10");
	Assert.assertEquals(usersMap.get(0).get("department_name"),"Administration");
